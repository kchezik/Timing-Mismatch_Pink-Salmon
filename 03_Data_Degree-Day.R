...
ggplot(final, aes(Degree.Days)) +
	geom_density(adjust = 8)

ggplot(final,aes(NULL,NULL)) +
	geom_point(aes(Degree.Days,Days.to.Emergence, colour = Treatment)) +
	geom_smooth(aes(Degree.Days,Days.to.Emergence), method = "lm", se = T)

saveRDS(final, file = "Data_02_Emergence.rds")


dat = readRDS("Data_02_Emergence.rds")
simple = dat %>% group_by(Treatment,Location,Source) %>% summarize(Degree.Days = mean(Degree.Days), Days.to.Emergence = mean(Days.to.Emergence))
simple = dat %>% select(Location, Treatment, Source, Northing, Easting) %>% distinct() %>% left_join(simple, ., by = c("Location","Treatment","Source"))

library(nlme)
mod = gls(model = Degree.Days~Days.to.Emergence, data = simple, weights = varExp(form = ~Days.to.Emergence))
#summary(mod)
#plot(residuals(mod, type = "normalized"))

intercept = coef(mod)[[1]]
slope = coef(mod)[[2]]
VarExp = mod$modelStruct[[1]][[1]]
sigma = mod$sigma

upper = intercept + slope*simple$Days.to.Emergence + 1.96 * sqrt(sigma^2 * exp(2*simple$Days.to.Emergence*VarExp))
lower = intercept + slope*simple$Days.to.Emergence - 1.96 * sqrt(sigma^2 * exp(2*simple$Days.to.Emergence*VarExp))

simple = simple %>% bind_cols(data.frame(upper.pred = upper, lower.pred = lower))
ggplot(simple,aes(NULL,NULL)) + 
	geom_point(aes(Days.to.Emergence,Degree.Days, colour = Treatment, shape = Source)) +
	geom_smooth(aes(Days.to.Emergence,Degree.Days), method = "lm", se = F) +
...
#Look for spatial gouping of DD data.
ggplot(simple, aes(Easting, Northing, colour = Location, size = Days.to.Emergence)) +
	geom_jitter(width = 100000, height = 100000)

ggplot(simple, aes(Easting, Northing, colour = Location, size = Degree.Days)) +
	geom_jitter(width = 100000, height = 100000)
