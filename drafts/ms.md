...
Estuary arrival overlap with the zooplankton bloom decreased with increasing climate divergence from the estuary (Figure $\ref{fig:mismatch}$). Although climate divergence was not perfectly predictive of zooplankton bloom overlap, the strongest overlap estimates among climatically similar locations were more than twice those among the most climatically divergent. Similarly, climate similar locations were less likely to have lower overlap values and never missed the zooplankton bloom completely while zero and near zero overlap were more prominant among increasingly climate divergent locations.

River distance from the estuary was strongly preditive of climate divergence and subsequently predictive of zooplankton bloom overlap (Figures $\ref{fig:mismatch}$ & $\ref{fig:chgmatch}$). Although climate divergence does not capture all the variation (5%), especially among low divergence values, the match-mismatch SSNM was able to capture much of the remaining variation (40%) in the $\mathrm{z}_{u}$ network spatial structure. LOOCV demonstrates that prediction standard error grows with observed overlap but never exceeds 15% (Sup. Figure 4) and even captures extreme values extending well beyond the mean estimate (Sup. Figure 5).

\begin{figure}[H]
\centering
\includegraphics[width=6.5in]{./99_figures/04_mismatch.pdf}
\caption{Percent overlap between jeuvenile pink salmon arrival and the zooplankton bloom, given how much natal stream climates diverge from the estuary. Points describe the overlap and climate divergence estimates for each population in each year data were available and are colored by river distance to the estuary. The mean effect of climate divergence on overlap given a fitted estimate of equation $\ref{eq4}$ is described by the solid black line.}
\label{fig:mismatch}
\end{figure}

On average phenological overlap is predicted to be greater as populations get closer to the estuary (Figure $\ref{fig:chgmatch}$). Stream temperature profiles and climate divergence estimates in the northwest region of the Fraser River watershed predict on average 1/3 the overlap (10%) of those near the estuary (30%). Differences between these two regions have been declining between 1970 and 2010 (Figure $\ref{fig:chgmatch}$). On average overlap in the lower Fraser and mainstem has declined while eastern and northwestern regions improved. No location has changed by more than 1.8%, well within prediction standard error estimates.
